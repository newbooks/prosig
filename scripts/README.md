...
`motif_cluster_scoreboard.pkl`. The default Plasma scale spans weights 0 to
10; hover text keeps the uncapped weight. In the HTML output, click a tile to
pin its details inside the tile.
...
to limit the display to the largest clusters. The selected motif's signature
is read from `prosig_motifs.tsv`; use `--motifs` to select another library.
